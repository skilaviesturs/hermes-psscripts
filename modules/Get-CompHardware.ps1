...
function Get-CompHardware {
    ...
    PROCESS {
        ...
        elseif ( $Api) {
            ...
            $api_login = "$_host/token";
            $api_get_status = "$_host$_api/status";
            
            $username = "supervisor@ltb.lan";
            $password = "supervisor@ltb.lan";
            ...
            $header_login = @{"Content-Type" = "application/x-www-form-urlencoded"}
            
            $response = Invoke-RestMethod -Uri "$api_login" -Method Post -Body $creds -Headers $header_login
            
            $token = $response.access_token;
            # JPB : Here, you receive the token so you use it
            $header_token = @{"Accept" = "application/json" ; "authorization" = "bearer $token"}
            Write-Host $header_token.authorization    
        }
        #>
        else {
            return $OutputObject
